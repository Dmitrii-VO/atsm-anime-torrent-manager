...
| 4 | Потоковый просмотр | **частично**: флаги есть, запуска плеера нет |
...
## Приоритет 4 — потоковый просмотр («Смотреть онлайн / стриминг») — ЧАСТИЧНО

Сделан первый шаг: `sequentialDownload` и `firstLastPiecePrio` уходят в клиент при
добавлении, под настройкой «Качать по порядку». Осталось то, что дороже, — пункт 3
сценария ниже: кнопка «Смотреть потоком», опрос прогресса и запуск плеера.
Переключатели для уже добавленных раздач тоже не задействованы.
