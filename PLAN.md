...
| 6 | RuTracker: поиск, скачивание, стриминг | **готово** |
...
## Приоритет 6 — RuTracker: поиск, скачивание, потоковый просмотр — ГОТОВО
...
### Этап 0 — спайк — ПРОЙДЕН 07.09.2026

Куки и UA из браузера подставлены в `requests`: поиск отдал 200 и 48
результатов при живой сессии, `dl.php` — настоящий `.torrent`
(`application/x-bittorrent`, 193 КБ), на странице темы нашёлся magnet.
Заглушки Cloudflare нет ни на одном из трёх запросов.

Значит подход рабочий и можно строить остальное. Осталось неизвестным, сколько
живёт `cf_clearance`: это выяснится только со временем, поэтому диагностика
«обновите куки» обязательна с первого дня.

Реализовано 07.09.2026, этапы 1–3 ниже закрыты. Отклонения от плана:
`read_password()` обобщены до `read_secret(key)` — куки легли в то же хранилище
по своему ключу; отдельного `RuTrackerSettings` не понадобилось, поля вошли в
`SourceSettings`, который реестр и так передаёт парсерам.
