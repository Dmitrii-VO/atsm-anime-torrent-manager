...
## Приоритет 1 — второй источник: AniLibria
...
## Приоритет 2 — правила выбора релизов (ТЗ §9)
...
## Приоритет 3 — замена устаревших пачек раздач в qBittorrent
...
## Приоритет 4 — потоковый просмотр («Смотреть онлайн / стриминг»)
...
## Приоритет 5 — поиск по названию (ТЗ §3, способ 2)

Опирается на эндпоинт поиска AniLibria (`/api/v1/app/search/releases`) и Shikimori.

## Приоритет 6 — источник RuTracker (ТЗ §20)

Добавление крупнейшего русскоязычного трекера как источника раздач.
Даёт доступ к огромной базе аниме: субтитры, многоголосые озвучки, BD-рипы, редкие тайтлы.

План реализации (Вариант А — встроенная авторизация):
- **Настройки источника** (`RuTrackerSettings` в `atsm/config.py`):
  - Логин и пароль аккаунта;
  - Зеркало / базовый адрес (по умолчанию `https://rutracker.org`, фоллбэк `rutracker.net`);
  - Настройка прокси (HTTP / SOCKS5, например, `socks5://127.0.0.1:10808` для обхода блокировок в РФ).
- **Авторизация**:
  - `POST /forum/login.php` с телом `login_username`, `login_password`, `login` в кодировке `windows-1251`.
  - Извлечение и сохранение сессионных кук `bb_session`, `bb_data`.
  - Автоматический повторный логин при истечении сессии (аналогично логике qBittorrent).
- **Парсер раздач** (`atsm/parsers/rutracker.py`):
  - Приём ссылок вида `https://rutracker.org/forum/viewtopic.php?t=<id>`.
  - Парсинг темы раздачи: название релиза, спойлер «Список файлов», кодеки/разрешение, сиды/личи.
  - Определение обновлений: в темах онгоингов на RuTracker торрент перезаливается при выходе новых серий (отслеживание по `info_hash` / дате обновления торрента).
- **Скачивание торрента**:
  - `GET /forum/dl.php?t=<id>` с сессионными куками отдаёт полноценный `.torrent`-файл.
  - Фоллбэк на magnet-ссылку со страницы темы (доступна даже без файла).

## Дешёвые победы, вне очереди

- **Импорт/экспорт JSON** (ТЗ §17) — без новых зависимостей, защищает накопленные подписки.
- **Пароль qBittorrent в Windows Credential Manager** — последний незакрытый пункт ТЗ §29.
